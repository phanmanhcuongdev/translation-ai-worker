...
class AITranslationService:
    ...
    async def translate(
        ...
        *,
        ...
        prompt = self._build_prompt(content=content, target_lang=target_lang)
        started_at = time.perf_counter()

        logger.info(
            "Calling Ollama model=%s target_lang=%s content_chars=%d",
        ...
        translated_text, source_lang = self._extract_translation_result(response)

        logger.info(
            "Ollama translation completed model=%s source_lang=%s target_lang=%s gpu_latency_ms=%.2f translated_chars=%d",
            self._settings.ollama_model,
            source_lang,
            target_lang,
            latency_ms,
    ...
    async def translate_to_vietnamese(self, content: str) -> TranslationResult:
        return await self.translate(content=content, target_lang="vi")

    @staticmethod
    def _build_prompt(*, content: str, target_lang: str) -> str:
        ...
        return (
            "You are a professional translation engine. Detect the source language "
            f"and translate the following content into {target_language_name}. "
            "If the source language is already the target language, return the "
            "original content unchanged as translated_text. Preserve meaning, "
            "terminology, punctuation, line breaks, markdown, URLs, and code blocks. "
            "Return a JSON object with exactly two string fields: "
            '"source_lang" and "translated_text". Use an ISO 639-1 code such '
            'as "vi" or "en" for source_lang. Do not return arrays, nested '
            "objects, markdown fences, or explanations.\n\n"
            f"Content:\n{content}"
        )

    @staticmethod
    def _extract_translation_result(response: Any) -> tuple[str, str]:
        ...
        if not isinstance(translated_text, str) or not translated_text.strip():
            raise AITranslationService._format_error(
                "Ollama response translated_text must be a non-empty string",
                response_text,
            )
        ...
        return translated_text.strip(), source_lang.strip().lower()[:16]
